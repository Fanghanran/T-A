  ...
  return (
    <div className="flex flex-col gap-2">
      {reports.map((a, ai) => {
        ...
        const questions = Array.isArray(r.interviewQuestions)
                  ...
                  <div className="rounded-lg border border-border/60 bg-secondary/30 p-3">
                    <div className="mb-1.5 flex items-center gap-2 text-[13px] font-medium">
                      ...
                      <span className="ml-auto">
                        <ChunkScoreBadge
                          score={r.jdMatch.score}
                          level={level(r.jdMatch.score)}
                          issues={['匹配度']}
                        />
                      </span>
                    </div>
                      ...
                      <div>
                        ...
                        <ul className="flex flex-col gap-1">
                          {(r.jdMatch.matched || []).map((m, i) => (
                            <li
                              key={i}
                              className="flex gap-1.5 text-xs text-emerald-700 dark:text-emerald-400"
                            >
                              <span>✓</span>
                              <span className="text-foreground/80">{m}</span>
                            </li>
                          ))}
                          {!(r.jdMatch.matched || []).length && (
