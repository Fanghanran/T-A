...
import { prepareDocChunksAndVectors, decodeFilename, extractDocumentTextAsync, attachChunkScores, attachChunkScoresAsync } from '../lib/docProcessor.js'
...
import { upload, jsonLimits, TEXT_EXT, UNSUPPORTED_HINT, parseTags, dbg } from './shared.js'
...
knowledgeRouter.post('/api/search/query', rateLimiters.search, jsonLimits.search, validateKnowledgeBody, async (req, res, next) => {
  try {
    const { q } = req.body ?? {}
...
// ---------- 文档上传 ----------
knowledgeRouter.post('/api/knowledge/documents', rateLimiters.upload, upload.single('file'), async (req, res, next) => {
  try {
    ...
    let extracted
    try {
      extracted = await extractDocumentTextAsync(req.file.buffer, originalName)
    } catch (e) {
    ...
    const chunkStrategy = req.body.chunkStrategy === 'delimiter' ? 'delimiter' : 'semantic'
    const delimiter =
      chunkStrategy === 'delimiter'
        ? typeof req.body.delimiter === 'string' && req.body.delimiter.length > 0
          ? req.body.delimiter
          : undefined
        : undefined
    const maxChars =
      chunkStrategy === 'delimiter' && req.body.maxChars ? Number(req.body.maxChars) : undefined
    // 相邻块滑动窗口重叠字符数（0 = 不重叠，默认 0）
    const overlapChars =
      chunkStrategy === 'delimiter' && req.body.overlapChars != null && req.body.overlapChars !== ''
        ? Number(req.body.overlapChars)
        : 0
    if (chunkStrategy === 'delimiter' && !delimiter) {
      return res.status(400).json({ message: 'chunkStrategy=delimiter 时 delimiter 必填' })
    }
    if (Number.isFinite(maxChars) && (maxChars < 50 || maxChars > 5000)) {
      return res.status(400).json({ message: 'maxChars 范围 50~5000' })
    }
    if (!Number.isInteger(overlapChars) || overlapChars < 0 || overlapChars > 500) {
      return res.status(400).json({ message: 'overlapChars 范围 0~500（整数）' })
    }
...
function _pruneJobs() {
  ...
  for (const [k, v] of uploadJobs) {
    if ((v.stage === 'done' || v.stage === 'error') && now - v.updatedAt > JOB_TTL_MS) {
      uploadJobs.delete(k)
    }
  }
}
...
knowledgeRouter.post('/api/knowledge/documents/prepare', rateLimiters.upload, upload.single('file'), async (req, res, next) => {
  try {
    if (!req.file) return res.status(400).json({ message: '缺少 file 字段' })
    const originalName = decodeFilename(req.file.originalname)
    if (!TEXT_EXT.test(originalName)) {
    ...
    const chunkStrategy = req.body.chunkStrategy === 'delimiter' ? 'delimiter' : 'semantic'
    const delimiter =
      chunkStrategy === 'delimiter' && typeof req.body.delimiter === 'string' && req.body.delimiter.length > 0
        ? req.body.delimiter
        : undefined
    const maxChars =
      chunkStrategy === 'delimiter' && req.body.maxChars ? Number(req.body.maxChars) : undefined
    ...
    const cfg = chunkStrategy === 'delimiter' && Number.isFinite(maxChars) ? { maxChars } : undefined
...
knowledgeRouter.post('/api/knowledge/documents/commit', jsonLimits.batch, async (req, res, next) => {
  try {
    const { previewId, category = '', tags, withQuestions } = req.body ?? {}
    if (!previewId || typeof previewId !== 'string') {
      return res.status(400).json({ message: '缺少 previewId（先调 /documents/prepare）' })
    }
    const entry = uploadPreviews.get(previewId)
    if (!entry) {
      return res.status(410).json({ message: '预览不存在或已过期（30 分钟），请重新上传' })
    }
    ...
    const run = async () => {
      ...
      const { chunkList, vectors } = await prepareDocChunksAndVectors(entry.text, {
      ...
      await store.addChunks(doc.id, chunkList, vectors, { category: safeCategory, tags: parsedTags })
      if (job) {
        job.stage = 'done'
        job.doc = doc
        job.updatedAt = Date.now()
...
knowledgeRouter.post('/api/knowledge/preview-chunks', jsonLimits.preview, async (req, res, next) => {
  try {
    const { text, strategy = 'semantic', delimiter, maxChars, overlapChars } = req.body ?? {}
    if (!text || typeof text !== 'string') {
      return res.status(400).json({ message: 'text 必填且必须为字符串' })
    }
    if (strategy === 'delimiter' && (typeof delimiter !== 'string' || delimiter.length === 0)) {
    ...
    let overlap = 0
    if (strategy === 'delimiter' && overlapChars != null && overlapChars !== '') {
...
const chunkScoreCache = createTtlLruCache({ maxEntries: 64, ttlMs: 10 * 60 * 1000, maxBytes: 8 * 1024 * 1024 })

function chunkSigKey(docId, rawChunks) {
  const sig = createHash('sha1').update(rawChunks.map((c) => `${c.id}:${(c.text || '').length}`).join('|')).digest('hex')
  return `${docId}::${sig}`
}
...
knowledgeRouter.get('/api/knowledge/documents', (req, res) => {
  ...
  const scoredItems = items.map((d) => ({ ...d, avgScore: listAvgScoreOfDoc(d.id) }))
...
knowledgeRouter.get('/api/knowledge/documents/:id/chunks', async (req, res, next) => {
  try {
    const doc = store.getDocument(req.params.id)
    if (!doc) return res.status(404).json({ message: '文档不存在' })
...
knowledgeRouter.patch('/api/knowledge/documents/:id', jsonLimits.chat, async (req, res, next) => {
  try {
    const id = req.params.id
    if (!store.getDocument(id)) return res.status(404).json({ message: '文档不存在' })
    const body = req.body ?? {}
    ...
    if ('title' in metaPatch) {
      if (typeof metaPatch.title !== 'string' || !metaPatch.title.trim() || metaPatch.title.length > 200) {
        return res.status(400).json({ message: 'title 长度 1~200 字符' })
      }
      meta.title = metaPatch.title
    }
    if ('category' in metaPatch) {
      meta.category = typeof metaPatch.category === 'string' ? metaPatch.category : ''
    }
    if ('tags' in metaPatch) {
      ...
      if (metaPatch.tags.some((t) => typeof t !== 'string' || t.length > 20)) {
        return res.status(400).json({ message: '每个 tag 长度 ≤ 20 字符' })
      }
      meta.tags = metaPatch.tags
    }
    if ('source' in metaPatch) {
      meta.source = typeof metaPatch.source === 'string' ? metaPatch.source : ''
    }
    ...
    if (typeof content === 'string') {
      if (content.trim().length < 10 || content.length > 500_000) {
        return res.status(400).json({ message: 'content 长度 10~500,000 字符' })
      }
      ...
      try {
        ...
        if (oldVecs.length > 0) {
          ...
          for (const c of oldVecs) {
            if (Array.isArray(c.vector) && c.vector.length > 0 && !reuse.has(c.text)) {
      ...
      const { chunkList, vectors, reusedCount } = await prepareDocChunksAndVectors(content, { reuse })
      if (reuse && reusedCount > 0) {
        dbg(`[PATCH content] 增量复用 ${reusedCount}/${chunkList.length} 块旧向量`)
      }
      doc = await store.updateContentWithPrepared(id, content, chunkList, vectors)
      if (!doc) return res.status(500).json({ message: '正文更新失败' })
    }
    // 两者都没带 → 400
    if (doc === null) {
      return res.status(400).json({
        message: '未携带任何可更新字段：可选 title / category / tags / source / content',
      })
    }
    res.json(doc)
  } catch (err) {
...
knowledgeRouter.get('/api/knowledge/orphans', (_req, res) => {
  res.json({ items: store.listOrphanDocs(), total: store.listOrphanDocs().length })
})

/** POST /api/knowledge/documents/:id/reindex —— 用存量正文重切+重嵌，补齐丢失的切片 */
knowledgeRouter.post('/api/knowledge/documents/:id/reindex', rateLimiters.upload, async (req, res, next) => {
  try {
    const { id } = req.params
    ...
    if (content.trim().length < 10) {
      return res.status(400).json({ message: '正文过短或为空，无法重新切片（请先删除该文档）' })
    }
    const { chunkList, vectors } = await prepareDocChunksAndVectors(content, {
      withQuestions: chunkerConfig?.questionsPerChunk > 0,
    })
    const updated = await store.updateContentWithPrepared(id, content, chunkList, vectors)
    if (!updated) return res.status(500).json({ message: '重新入库失败' })
    dbg(`[reindex] 文档 ${id} 重切片入库完成：${chunkList.length} 块`)
    res.json({ id, title: updated.title, chunkCount: chunkList.length, status: updated.status })
  } catch (err) {
    next(err)
  }
})
...
knowledgeRouter.post('/api/knowledge/orphans/reconcile', jsonLimits.small, async (req, res, next) => {
  try {
    const orphans = store.listOrphanDocs()
    const limit = Number.isFinite(Number(req.body?.limit)) ? Math.min(Number(req.body.limit), 50) : 50
    const results = []
    for (const o of orphans.slice(0, limit)) {
      try {
        const { chunkList, vectors } = await prepareDocChunksAndVectors(o.contentLen ? (store.getDocument(o.id)?.content ?? '') : '', {
          withQuestions: chunkerConfig?.questionsPerChunk > 0,
        })
        await store.updateContentWithPrepared(o.id, store.getDocument(o.id)?.content ?? '', chunkList, vectors)
        results.push({ id: o.id, ok: true, chunkCount: chunkList.length })
      } catch (e) {
        results.push({ id: o.id, ok: false, error: e?.message || 'reindex 失败' })
      }
    }
...
knowledgeRouter.post('/api/knowledge/documents/batch', jsonLimits.batch, async (req, res, next) => {
  try {
    ...
    switch (op) {
      ...
      case 'setCategory': {
        ...
        const result = await store.batchPatchMetaAsync(safeIds, { setCategory: category })
        return res.json({ op, ...result, category })
      }
      case 'addTags': {
        const tags = Array.isArray(req.body.tags) ? req.body.tags : []
        if (tags.length === 0) return res.status(400).json({ message: 'tags 必填' })
        if (tags.some((t) => typeof t !== 'string' || t.length > 20)) {
          return res.status(400).json({ message: '每个 tag 长度 ≤ 20 字符' })
        }
        const result = await store.batchPatchMetaAsync(safeIds, { addTags: tags })
        return res.json({ op, ...result })
      }
      case 'removeTag': {
        const tags = Array.isArray(req.body.tags) ? req.body.tags : []
        if (tags.length === 0) return res.status(400).json({ message: 'tags 必填' })
        const result = await store.batchPatchMetaAsync(safeIds, { removeTags: tags })
        return res.json({ op, ...result })
      }
      default:
        return res.status(400).json({
...
// ---------- 分类 / 标签 ----------
knowledgeRouter.get('/api/knowledge/categories', (_req, res) => res.json(store.listCategories()))
knowledgeRouter.get('/api/knowledge/tags', (_req, res) => res.json(store.listTags()))

// ---------- 分类治理：重命名（to='' 表示并入「未分类」）----------
knowledgeRouter.post('/api/knowledge/categories/rename', jsonLimits.small, async (req, res, next) => {
  try {
    const from = typeof req.body?.from === 'string' ? req.body.from.trim() : ''
    const to = typeof req.body?.to === 'string' ? req.body.to.trim() : ''
    if (!from) return res.status(400).json({ message: 'from 必填（原分类名）' })
    if (to.length > 50) return res.status(400).json({ message: 'to 长度 ≤ 50 字符' })
    ...
    res.json({ renamed: r.updated.length, affected: r.updated, failed: r.failed })
  } catch (err) {
    next(err)
  }
})
...
knowledgeRouter.post('/api/knowledge/tags/merge', jsonLimits.small, async (req, res, next) => {
  try {
    ...
    const from = (Array.isArray(fromRaw) ? fromRaw : [fromRaw])
      ...
      .filter(Boolean)
    const to = typeof req.body?.to === 'string' ? req.body.to.trim() : ''
    if (from.length === 0) return res.status(400).json({ message: 'from 必填（待合并标签，字符串或数组）' })
    if (!to) return res.status(400).json({ message: 'to 必填（合并后的目标标签）' })
    if (to.length > 20) return res.status(400).json({ message: 'to 长度 ≤ 20 字符' })
    ...
    const r = await store.batchPatchMetaAsync(affected, { removeTags: from, addTags: [to] })
    res.json({ merged: r.updated.length, from, to, affected: r.updated, failed: r.failed })
  } catch (err) {
    next(err)
  }
})
...
knowledgeRouter.post('/api/knowledge/duplicates/scan', jsonLimits.small, async (req, res, next) => {
  try {
...
knowledgeRouter.post('/api/knowledge/duplicates/delete', jsonLimits.small, async (req, res, next) => {
  try {
    const chunkIds = Array.isArray(req.body?.chunkIds) ? req.body.chunkIds : []
    if (chunkIds.length === 0) return res.status(400).json({ message: '缺少 chunkIds（非空数组）' })
...
knowledgeRouter.post('/api/knowledge/documents/manual', jsonLimits.manual, async (req, res, next) => {
  try {
    ...
    if (typeof content !== 'string' || content.trim().length < 10 || content.length > 500_000) {
      return res.status(400).json({ message: 'content 必填，长度 10~500,000 字符' })
    }
    ...
    await store.addChunks(doc.id, chunkList, vectors, { category: safeCategory, tags: safeTags })
    res.status(201).json(doc)
  } catch (err) {
    next(err)
  }
...
knowledgeRouter.post('/api/knowledge/search', rateLimiters.search, validateKnowledgeBody, async (req, res, next) => {
  try {
...
knowledgeRouter.post('/api/knowledge/ask', rateLimiters.search, validateKnowledgeBody, async (req, res, next) => {
  try {
    const { query, category, tag, history } = req.body ?? {}
    if (!query?.trim()) return res.status(400).json({ message: '缺少 query' })
    const t0 = performance.now()
    ...
    const searchMs = Math.round(performance.now() - t0)
    pipeStream(res, await streamRagAnswer({ query, chunks, searchMs, history }))
  } catch (err) {
    next(err)
  }
})
